...
export default function BoseSoundTouchController() {
  ...
  const connectToDevice = async (ip) => {
    try {
      const response = await fetch(`http://${ip}:8090/info`);
      if (response.ok) {
        setConnected(true);
        setError(null);
        fetchPresets(ip);
  ...
  const fetchVolume = async (ip) => {
    try {
      const response = await fetch(`http://${ip}:8090/volume`);
  ...
  const fetchNowPlaying = async (ip) => {
    try {
      const response = await fetch(`http://${ip}:8090/now_playing`);
      const text = await response.text();
      const parser = new DOMParser();
      const xml = parser.parseFromString(text, 'text/xml');
  ...
  const fetchPresets = async (ip) => {
    try {
      const response = await fetch(`http://${ip}:8090/presets`);
      const text = await response.text();
      const parser = new DOMParser();
      const xml = parser.parseFromString(text, 'text/xml');
  ...
  const sendKey = async (key) => {
    ...
    try {
      await fetch(`http://${deviceIP}:8090/key`, {
        method: 'POST',
  ...
  const setVolumeLevel = async (newVolume) => {
    ...
    try {
      await fetch(`http://${deviceIP}:8090/volume`, {
        method: 'POST',
        headers: { 'Content-Type': 'text/xml' },
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-2">
                Find this in your router settings or Bose app
              </p>
            </div>
